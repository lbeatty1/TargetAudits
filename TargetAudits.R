...
#define stuff #
###############
gridsize = 0.1 #for now just make grid thats lat/lons rather than dist
...
budget = 10000 #size of audit budget

n_bins = 5 #size of grid emission and grid count bins
...
start=rep(0.01, nrow(prob_by_bin))
sol = nloptr(start,
...
ggplot(prob_by_bin%>%mutate(count_bin=as.numeric(count_bin), emis_bin=as.numeric(emis_bin))) +
  ...
  coord_fixed()+
  theme_bw()
ggsave(filename = paste('gridsize', gridsize, '/audit_probabilities_nbins', n_bins, '.jpg', sep=''),
       width=8,
       height=6,
       device='jpeg')
...
ggplot(emis_grid_target)+
  ...
  scale_fill_viridis_b()+
  theme_bw()
ggsave(filename=paste('gridsize', gridsize, '/bins', n_bins, '_grid_map_prob_CONUS.jpg', sep=''),
       width=8,
       height=6,
       device='jpeg')
...
ggplot(emis_grid_target)+
  ...
  coord_sf(xlim=c(-104,-100), ylim=c(30,34))+
  theme_bw()
ggsave(filename=paste('gridsize', gridsize, '/bins', n_bins, '_grid_map_prob_PERMIAN.jpg', sep=''),
       width=8,
       height=6,
       device='jpeg')

write.table(t(c(gridsize, n_bins, budget, sum(emis_uniform$detected), detected_targeted, audits_targeted)),
          file = 'output.csv',
